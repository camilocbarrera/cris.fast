    ...
    >
      <div className="flex">
        {showDays && (
          ...
          >
            {showMonths && <div style={{ height: MONTH_LABEL_HEIGHT }} />}
            <div className="relative" style={{ height: gridHeight }}>
              {DAY_LABELS.map(({ label, row }) => (
                <span
                  key={label}
                  className="absolute right-2 text-[13px] text-muted-foreground leading-none"
        ...
        <div className="relative">
          {showMonths && (
            <div className="relative" style={{ height: MONTH_LABEL_HEIGHT }}>
              {monthLabels.map(({ month, xOffset }, i) => (
                <span
                  key={`${month}-${i}`}
                  className="absolute text-[13px] text-muted-foreground leading-none"
                  style={{ left: xOffset }}
                >
                  {month}
                </span>
              ...
              >
                {showCounts && day.count > 0 && (
                  <span
                    ...
                    style={{ fontSize: `${Math.max(cellSize - 4 - (String(day.count).length - 1), 6)}px` }}
                  >
                    {day.count}
                  </span>
                )}
